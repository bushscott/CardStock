...
- **Pipeline order (load-bearing):** forwarded-headers → host filtering → security headers → rate limiter → app. Cap and logs both see true IPs. (Host filtering actually runs in the host's startup filter ahead of the whole app pipeline — behaviorally equivalent here, since only X-Forwarded-For is enabled and the Host header is never rewritten; do not "fix" the order.)
