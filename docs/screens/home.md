...
| 3 | `Tier` | 52px | **yes** | Grade tier of the tracked row — **part of the row's identity**, see below | Mono 12.5px, centred. **Build values: `PSA 10`, `Grade 9`, `Grade 8`, `Raw`.** The prototype seeds `PSA 9` / `PSA 8`, which are not real tiers — ADR-0005 pools graders below 10 (D-055) | `sales.grade_tier` vocabulary |
...
| 2 | `LOW CONFIDENCE` is a **Charts-only** badge for burned-in / show-anyway rows, distinct from the `LOW DATA` sufficiency state | DN:33, DN:131, DN:146 | Home's feed uses `LOW CONFIDENCE` as the name of the state a **newly unlocked indicator starts in** — *"per-sale ledger reached 30 days for PSA 10 — starts LOW CONFIDENCE"* (line 423). **✅ RESOLVED 2026-08-10 by D-056 — plain `LOW CONFIDENCE` is abolished; it was `LOW DATA` under a second name.** Build this feed row as *"per-sale ledger reached 30 days for PSA 10 — **starts LOW DATA**"*. `LOW CONFIDENCE · BURNED IN` survives as the user-override marker only. `DN:33/:131/:146`'s "Charts-only" framing was stale regardless — `Charts:576` already applied the plain form automatically |
...
| 5 | *(consequence of #4)* — implied: the peek highlights the row matching the card's tier | — | The match is raw string equality against the six labels (line 511). For the two `PSA 9` / `PSA 8` cards **nothing highlights**, because `'PSA 9' !== 'Grade 9'`. A live defect caused directly by the vocabulary split. **✅ RESOLVED 2026-08-10 by D-055 — the labels are wrong, not the matcher.** Build `Grade 9` / `Grade 8`; the highlight then works without changing the comparison. Do not "fix" this by loosening the string match — that would paper over a vocabulary error with a fuzzy comparison |
...
| 21 | *"**One row per card + tier** on watchlists"* | HO:155, DN:110 | The prototype's row key is the **card id alone** — `lists[].ids` holds bare ids (lines 412–415) and the tier is read off the card record (line 556). The (card, tier) pair is **not representable**: the same card cannot appear twice at two tiers. The rule is satisfied only because the model cannot express a violation. **✅ RESOLVED 2026-08-10 by D-055 — the docs win: build the key as `(card_id, tier)`.** Signals are tier-specific, and grading arbitrage (`Psa10 ÷ Ungraded`) requires watching one card at two tiers deliberately. The Card page picker adds at a default tier; the tier is edited from the row's ⋯ menu or from Charts, which `DN:112` already establishes as the editor |
