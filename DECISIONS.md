...
- [x] ~~WireGuard~~ **VOID by owner ruling, 2026-08-20 (final amendment):** no static IP after all — the tunnel proved outbound-only serves the site, so **remote management = Raspberry Pi Connect** (`rpi-connectd`, outbound relay, already running; the pre-move sweep's keep-or-disable decide-point resolves as *keep, as the sole remote door*). Under CGNAT with no static IP, no inbound endpoint can ever exist: WireGuard, DDNS, and the ISP call are all permanently off the list, and §F's structural unreachability receipts become permanent rather than provisional. ACL rule 4 + the `WG-VPN-Range` group stay dormant (harmless; revive only if a routable address ever exists). Trade-offs on record: Pi Connect is a browser shell — no rsync/scp/DB tunnel remotely, so deploys and DB test runs stay at-home; the Raspberry Pi ID becomes the management credential (owner asked to enable 2FA on it). Next home session: verify `rpi-connect` status
...
- [x] Receipt: site loads on a phone off wifi ✓ owner journey 2026-08-20 (browse · card · chart · ledger) — rides §F's phone-journey receipt
...
- [x] Resend: account · add domain · publish issued DKIM/SPF records · apex SPF `v=spf1 -all` stays · DMARC `p=reject` · null MX · test send to gmail passes DKIM+DMARC (receipt: message headers) — **everything but the send ✓ 2026-08-20**: Resend's Cloudflare integration published the records (DKIM at `resend._domainkey`; its SPF/MX confined to the `send.` subdomain it uses for envelopes), the zone audit confirmed the apex locks untouched, and Resend's Domains page shows **Verified**. **Send completed 2026-08-20** (owner's key, Resend id `2e14cc6b`): one message from `hello@cardstock.pro` to iCloud + Gmail, **delivered to both inboxes** — and under `p=reject` at two strict receivers, inbox delivery *is* the auth verdict (an unauthenticated claimant is refused, not delivered). Recipient amended to include iCloud at owner request; Gmail's Show-original stays available for the literal three-PASS table
...
- [x] securityheaders.com: **A** ✓ owner ran it in-browser 2026-08-20 and reports the letter: **A**. (The site 403s every non-browser client — curl bare and with a browser UA — so the grade is owner-eyes-only by construction.) The raw material is live and complete (CSP, XCTO, XFO + frame-ancestors, Referrer-Policy, Permissions-Policy; STS deliberately absent until §G)
...
- [x] Full journey on the phone: browse, chart, ledger, refresh ✓ owner, 2026-08-20 ("1 and 2 good") — §D's phone box ticked in step
